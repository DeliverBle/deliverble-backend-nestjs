...
export class UserController {
  ...
  async getUserInfo(@Req() req, @Res() res): Promise<Response> {
    try {
  ...
  async toggleFavoriteNews(
    ...
    try {
      ...
      const addOrDelete: TOGGLE_FAVORITE =
        await this.userService.toggleFavoriteNews(user, newsId);
  ...
  @Get('/load')
  @UseGuards(JwtAuthGuard)
  async loadUserInfo(@Req() req): Promise<User> {
    // console.log(req);
    return req.user;
  }
}
