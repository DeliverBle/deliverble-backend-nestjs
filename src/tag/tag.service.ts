...
export class TagService {
  ...
  async getAllTags(): Promise<ReturnTagDtoCollection> {
    ...
    const returnTagDtoCollection: ReturnTagDtoCollection =
      new ReturnTagDtoCollection(tagList);
    return returnTagDtoCollection;
  }

  async deleteTag(tagName: string): Promise<ReturnTagDto> {
    const returnNewsDto: ReturnTagDto = await this.tagRepository.deleteTag(
      tagName,
    );
