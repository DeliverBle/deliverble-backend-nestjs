...
export class AuthController {
	...
	kakaoLoginGetCode(@Res() res): void {
		...
		// 사용자 정보 존재 확인(수동))
		@Get('/kakao/login/user/is')
		@Header('Content-Type', 'text/html')
		async kakaoLoginGetUserIs(@Body() body, @Res() res): Promise<Response> {
			
			const socialId = body.socialId;
			// logger.debug(socialId);
			
			try {
				const response = await this.authService.checkUserIs(socialId);
				logger.debug('check >>>>', response);
				return res.send('Check user information complete');
		...
		async kakaoLoginGetUserIsByCode(@Query() qs, @Res() res): Promise<Response> {
			...
			try {
				const response = await this.authService.kakaoLoginOrSignUp(code);
				logger.debug('check >>>>', response);
				return res.send('Check user information complete');
